...
class HighlevelDirectEnv(DirectRLEnv):
    ...
    def __init__(
        ...
        self.previous_sweeping_action_column = torch.zeros(self.num_envs, 1, device=self.device) # 이전 step에서 sweeping 해서 물체가 옮겨진 위치의 column
        self.max_dis_column = torch.zeros(self.num_envs, 1, device=self.device) # 4개의 column 중 가장 큰 distribution 값의 column index
    ...
    def _get_observations(self) -> dict:
        ...
        shelf_front_object = self.shelf_object_config.gather(dim=1, index=max_row_indices.unsqueeze(1)).squeeze(1) # shape: (num_envs, num_cols)
